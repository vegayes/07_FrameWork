...
memberEmail.addEventListener("input", () => {
    ...
    if(  regEx.test(memberEmail.value)  ){ // 유효한 경우
        ...
        // Post 방식 이용 
        fetch("/dupCehck/email",{
            method : "POST",
            headers : {"Content-Type" : "application/json"},
            body : JSON.stringify({"emailCheck" : memberEmail.value})
        })
        .then(resp => resp.text())
        .then(member => {
            // 정수로 받아오는 방법
            if(member == ""){
                console.log("null값 입니다.");
...
memberNickname.addEventListener("input", ()=>{
    ...
    if(regEx.test(memberNickname.value)){// 유효
        ...
        // Get 방식 이용 
        fetch("/dupCehck/nickname?nickname=" + memberNickname.value)
        .then(resp => resp.text())
        .then(nickname => {
            if(nickname == ""){
                console.log(" nickname null값 입니다.");
                ...
                nickMessage.innerText = "닉네임이 중복되었습니다.";
